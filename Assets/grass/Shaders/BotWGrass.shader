Shader "Custom/BotWGrass"
{
	Properties
	{
		...
		_UseTextureColor("Use Texture Color", Float) = 1 // 1 for true, 0 for false
		...
		_BladeSegments("Blade Segments", Range(1, 50)) = 3
		_BladeBendDistance("Blade Forward Amount", Float) = 0.38
		_BladeBendCurve("Blade Curvature Amount", Range(1, 100)) = 2
	...
	SubShader
	{
		...
		HLSLINCLUDE
			...
			float customRand(float3 position)
			{
				return frac(sin(dot(position, float3(12.9898, 78.233, 45.5432))) * 43758.5453);
			}
			...
			void geom(point VertexOutput input[1], inout TriangleStream<GeomData> triStream)
			{
				...
				if (grassVisibility >= _GrassThreshold)
				{
					...
					float width = lerp(_BladeWidthMin, _BladeWidthMax, customRand(pos.xzy) * falloff);
					float height = lerp(_BladeHeightMin, _BladeHeightMax, customRand(pos.zyx) * falloff);

					int segments = int(_BladeSegments);
					for (int i = 0; i < segments; ++i)
					{
						float t1 = i / (float)segments;
						float t2 = (i + 1) / (float)segments;

						float3 offsetBottom = float3(width * 0.5, 0, height * t1); // Width divided by 2 for symmetry
						float3 offsetTop = float3(width * 0.5, 0, height * t2);     // Width divided by 2 for symmetry

						// Generate a random rotation matrix for this segment
						float3x3 randBendMatrix = angleAxis3x3(customRand(pos.zzx) * _BendDelta * UNITY_PI * 0.5f, normalize(float3(customRand(pos), customRand(pos.yxz), customRand(pos.zxy))));

						float3x3 transformationMatrix = mul(tipTransformationMatrix, randBendMatrix);

						float2 uvBottomLeft = float2(0, t1);
						float2 uvBottomRight = float2(1, t1);
						float2 uvTopLeft = float2(0, t2);
						float2 uvTopRight = float2(1, t2);

						// First triangle of the rectangle
						triStream.Append(TransformGeomToClip(pos, float3(-offsetBottom.x, offsetBottom.y, offsetBottom.z), transformationMatrix, uvBottomLeft));
						triStream.Append(TransformGeomToClip(pos, offsetBottom, transformationMatrix, uvBottomRight));
						triStream.Append(TransformGeomToClip(pos, float3(-offsetTop.x, offsetTop.y, offsetTop.z), transformationMatrix, uvTopLeft));

						// Second triangle of the rectangle
						triStream.Append(TransformGeomToClip(pos, offsetBottom, transformationMatrix, uvBottomRight));
						triStream.Append(TransformGeomToClip(pos, offsetTop, transformationMatrix, uvTopRight));
						triStream.Append(TransformGeomToClip(pos, float3(-offsetTop.x, offsetTop.y, offsetTop.z), transformationMatrix, uvTopLeft));
					}
				}
			}
        ...
        Pass
        {
			...
			{
				float4 textureColor = tex2D(_BladeTexture, i.uv);
				float4 baseColor = lerp(_BaseColor, _TipColor, i.uv.y);

				// Apply texture color based on the boolean
				float4 color = (_UseTextureColor > 0.5) ? (baseColor * textureColor) : baseColor;

				// Discard fragments below the alpha clipping threshold
				if (textureColor.a < _AlphaClipThreshold)
				{
					discard;
				}
				...
				#else
				half shadowAttenuation = 1.0; // No shadow attenuation if shadows are not defined
				#endif
